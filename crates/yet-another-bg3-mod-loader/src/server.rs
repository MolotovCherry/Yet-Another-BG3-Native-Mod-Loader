    ...
    loop {
        let cb = |cmd| match cmd {
            Command::Log(mut msg) => {
                ...
                let target = format!("dll.{}", msg.target);
                let span = msg.span;
                let spans = msg.spans;
                ...
                match msg.level {
                    Level::Trace => {
                        trace!(%target, %filename, line_number, ?span, ?spans, ?fields, "{message}")
                    }

                    Level::Debug => {
                        debug!(%target, %filename, line_number, ?span, ?spans, ?fields, "{message}")
                    }

                    Level::Info => {
                        info!(%target, %filename, line_number, ?span, ?spans, ?fields, "{message}")
                    }

                    Level::Warn => {
                        warn!(%target, %filename, line_number, ?span, ?spans, ?fields, "{message}")
                    }

                    Level::Error => {
                        error!(%target, %filename, line_number, ?span, ?spans, ?fields, "{message}")
                    }
                }
            }
